...
module dewpoint_mod
  ...
  function dewpoint(T,rh,rhl,rhs,return_fp,return_max_dp_fp)
     ...
     logical, intent(in), optional :: return_fp, return_max_dp_fp
     ...
     double precision :: Ttrip, ptrip, E0v, E0s, ggr, rgasa, &
        ...
        pv, qv, cpm, rgasm, rh2, rhl2, rhs2, Td, Tf, l1, l2
     integer :: rh_counter
     logical :: return_fp2, return_max_dp_fp2
  
     ! Set defaults for options
     return_fp2        = .false.
     return_max_dp_fp2 = .false.
  
     if (present(return_fp)) return_fp2 = return_fp
     if (present(return_max_dp_fp)) return_max_dp_fp2 = return_max_dp_fp
     ...
     if (return_fp2 .and. return_max_dp_fp2) then
        print *,'Error in dewpoint: At most, only one of return_fp and return_max_dp_fp can be true'
        stop
     end if
     ...
     if (rh_counter .ne. 1) then
        print *,'Error in dewpoint: Exactly one of rh, rhl, and rhs must be specified'
        stop
     end if
     if (present(rh)) then
        rh2 = rh
     ...
     cl = bl/al
     Td = T*cl/wapr(rhl2**(1./al)*cl*exp(cl),-1,error,0)
     as = -(cpv-cvs)/rgasv
     bs = -(E0v+E0s-(cvv-cvs)*Ttrip)/(rgasv*T)
     cs = bs/as
     l1 = log(rhs2)/as + log(cs) + cs
     if (rh2 .eq. 0.) then
        Tf = 0.
     else if (l1 .lt. 709.) then
        Tf = T*cs/wapr(rhs2**(1./as)*cs*exp(cs), 0,error,0)
     else
        l2 = log(l1)
        Tf = T*cs/(l1-l2+l2/l1+l2*(-2.+l2)/(2.*l1*l1)+l2*(6.-9.*l2+2.*l2*l2)/(6.*l1*l1*l1)+ &
           l2*(-12.+36.*l2-22.*l2*l2+3.*l2*l2*l2)/(12.*l1*l1*l1*l1))
     end if
  
     ! Return either Td or Tf
     if (return_fp2 .and. return_max_dp_fp2) then
        print *,'Error in dewpoint: return_fp and return_max_dp_fp cannot both be true'
        stop
     else if (return_fp2) then
        dewpoint = Tf
     else if (return_max_dp_fp2) then
        dewpoint = max(Td,Tf)
     else
        dewpoint = Td
     end if
...
function lcl(p,T,rh,rhl,rhs,return_ldl,return_min_lcl_ldl)
   ...
   if (present(rh)) then
      rh2 = rh
      ! The variable rh is assumed to be 
      ! with respect to liqusid if T > Ttrip and 
      ! with respect to solid if T < Ttrip
      if (T > Ttrip) then
         pv = rh2 * pvstarl(T)
      else
...
! 以上取自https://romps.berkeley.edu/papers/pubs-2016-lcl.html
...
program lcl_main
  ...
  use dewpoint_mod
  implicit none
  logical :: return_ldl, return_min_lcl_ldl, return_fp, return_max_dp_fp
  double precision :: input_p, input_T, input_rh
  double precision :: pres_Pa, temp_K
  double precision :: p, T, rh, rhl, rhs, Td   !hPa, Cels, perc, perc, perc, Kel
  character*3 :: y_or_n
  !---------------------------   

  001 continue

    write( *,"('輸入地表氣壓(hPa), 溫度(Celsius), 相對濕度(%)')" )
    read(*,*) input_p, input_T, input_rh

    pres_Pa = input_p * 100.
    temp_K = input_T + 273.15
    ...
    !------------------ dew point
    return_fp        = .false.
    return_max_dp_fp = .false.

    Td = dewpoint(T,rh,rhl,rhs,return_fp,return_max_dp_fp)


    !------------------ output result
    write( *,"(' ------------------------------------------------------------')" )
    write( *,"('｜  LCL = ',f6.3,' km (by exact expression)                   ｜')" ) &
             0.001 * lcl(p,T,rh,rhl,rhs,return_ldl,return_min_lcl_ldl)
    write( *,"('｜  LCL = ',f6.3,' km (for 層狀雲 by approximate expression)  ｜')" ) 0.124*(T-Td)
    write( *,"('｜  LCL = ',f6.3,' km (for 對流雲 by approximate expression)  ｜')" ) 0.208*(T-Td)
    write( *,"(' ------------------------------------------------------------')" )

    write(*,'(A)') NEW_LINE('A')//'Want to continue? [y/n]'
    read(*,*) y_or_n

    if (y_or_n .eq. 'n' .or. y_or_n .eq. 'N' .or. y_or_n .eq. 'no') then
      write(*,'(A)') NEW_LINE('A')//'stopped'
      stop
    else if (y_or_n .eq. 'y' .or. y_or_n .eq. 'Y' .or. y_or_n .eq. 'yes') then
      write(*,'(A)') NEW_LINE('A')
      goto 001
    end if
